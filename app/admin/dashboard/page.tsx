...
import { PlusCircle, Mail, Shield, CheckCircle, Clock, Play, Activity, Target, Zap, RefreshCw } from 'lucide-react'
import { AlertCircle, AlertTriangle } from 'lucide-react'
import { useActiveBusinessStore } from '@/lib/store/active-business-store'
import { getDashboardStatsAction, getActiveExecutionsAction, getRecentExecutionsAction, getExecutionClientsAction } from '@/lib/actions/collection/execution'
...
export default function DashboardPage() {
  ...
  const [reputationProfiles, setReputationProfiles] = useState<EmailReputationProfile[]>([])
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const [statsData, activeExecs, reputationData, recentData] = await Promise.all([
          getDashboardStatsAction(activeBusiness.id),
          getActiveExecutionsAction(activeBusiness.id),
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'collection_executions', filter: `business_id=eq.${activeBusiness.id}` },
        () => {
  ...
  const setActiveExecutionClientStats = (executionId: string, clientStats: ActiveExecution['clientStats']) => {
    ...
    return (
      <div className="flex flex-col gap-6 w-full overflow-auto">
        <div>
          ...
          <p className="text-sm sm:text-base text-muted-foreground">Selecciona una sucursal para ver el tablero</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 w-full h-full min-h-[calc(100vh-120px)]">
      <div className="shrink-0 flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Tablero - Cobros</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={manualRefresh} disabled={statsLoading}>
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <CardDescription>{exec.strategy_type === 'immediate' ? 'Inmediata' : 'Programada'}</CardDescription>
                    </div>
        ...
        <Card>
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{(stats?.total_emails_sent || 0).toLocaleString()}</div>
                <div className="flex items-center gap-2">
                  <p className="text-xs text-muted-foreground">Hoy: {(stats?.today_emails_sent || 0).toLocaleString()}</p>
                  {stats && stats.today_emails_sent > 0 && (
                    <Badge variant="outline" className="text-xs">{((stats.today_emails_sent / Math.max(stats.total_emails_sent, 1)) * 100).toFixed(0)}%</Badge>
                  )}
                </div>
              </>
            )}
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{(stats?.total_emails_opened || 0).toLocaleString()} abiertos</p>
              </>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{(stats?.total_emails_delivered || 0).toLocaleString()} de {(stats?.total_emails_sent || 0).toLocaleString()}</p>
              </>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                <div className={`text-2xl font-bold ${(stats?.avg_bounce_rate || 0) > 5 ? 'text-red-600' : (stats?.avg_bounce_rate || 0) > 2 ? 'text-yellow-600' : 'text-green-600'}`}>
                  {stats?.avg_bounce_rate || 0}%
                </div>
                <p className="text-xs text-muted-foreground">{(stats?.total_emails_bounced || 0).toLocaleString()} rebotados</p>
              </>
            )}
          </CardContent>
        </Card>
            ...
            {statsLoading ? (
              ...
              <>
                ...
                <div className="flex gap-2 text-xs mt-1">
                  <Badge variant="outline" className="text-xs">{stats?.completed_executions || 0} completadas</Badge>
                  {stats?.failed_executions ? <Badge variant="destructive" className="text-xs">{stats.failed_executions} fallidas</Badge> : null}
                </div>
              </>
            )}
          </CardContent>
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{stats?.pending_executions || 0} pendientes</p>
              </>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {statsLoading ? (
              ...
              <>
                <div className="text-sm font-bold">{formatDate(stats?.last_execution_date)}</div>
                <p className="text-xs text-muted-foreground">
                  {stats?.total_executions ? `${stats.total_executions} ejecuciones` : 'Sin ejecuciones'}
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {reputationLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{reputationProfiles.length}</div>
                <p className="text-xs text-muted-foreground">
                  {reputationProfiles.filter(p => p.is_warmed_up).length} activos, {reputationProfiles.filter(p => !p.is_warmed_up).length} en warm-up
                </p>
              </>
            )}
          </CardContent>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge className={`text-xs ${getStatusColor(exec.status)}`}>{exec.status}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {exec.total_clients} clientes • {exec.emails_sent} enviados • {exec.emails_opened || 0} abiertos
                        </p>
                      </div>
                      <div className="text-right text-sm text-muted-foreground">
                        <p>{formatDate(exec.created_at)}</p>
                        <p className="text-green-600">{exec.avg_open_rate?.toFixed(1) || 0}% opens</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        {reputationLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="py-8 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No hay dominios configurados</h3>
              <p className="text-muted-foreground mb-4">Los dominios se crean automáticamente cuando envías tu primera campaña</p>
              <Link href="/admin/settings/email-delivery">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {reputationProfiles.map((profile) => (
              <Card key={profile.id} className={profile.has_reputation_issues ? 'border-red-300' : ''}>
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <CardTitle className="text-lg font-semibold">{profile.domain}</CardTitle>
                      <CardDescription className="text-xs">
                        {profile.is_warmed_up ? (
                          <span className="text-green-600 flex items-center gap-1">
                            <CheckCircle className="h-3 w-3" /> Warm-up completado
                          </span>
                        ) : (
                          <span className="text-yellow-600 flex items-center gap-1">
                            <Clock className="h-3 w-3" /> Día {profile.current_warmup_day} de warm-up
                          </span>
                        )}
                      </CardDescription>
                    </div>
                    <Badge variant={profile.is_warmed_up ? 'default' : 'secondary'}
                      className={profile.is_warmed_up ? 'bg-green-100 text-green-800' : ''}>
                ...
                <CardContent className="space-y-3">
                  {!profile.is_warmed_up && (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">Progreso</span>
                        <span className="font-medium">{Math.min((profile.current_warmup_day / 6) * 100, 100).toFixed(0)}%</span>
                      </div>
                      <Progress value={Math.min((profile.current_warmup_day / 6) * 100, 100)} className="h-2" />
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-muted rounded p-2">
                      ...
                      <p className={`font-semibold text-sm ${profile.delivery_rate >= 95 ? 'text-green-600' : profile.delivery_rate >= 90 ? 'text-yellow-600' : 'text-red-600'}`}>
                        {profile.delivery_rate.toFixed(1)}%
                      </p>
                    </div>
                    <div className="bg-muted rounded p-2">
                      <p className="text-xs text-muted-foreground">Opens</p>
                      <p className={`font-semibold text-sm ${profile.open_rate >= 20 ? 'text-green-600' : profile.open_rate >= 10 ? 'text-yellow-600' : 'text-red-600'}`}>
                        {profile.open_rate.toFixed(1)}%
                      </p>
                    </div>
                    <div className="bg-muted rounded p-2">
                      <p className="text-xs text-muted-foreground">Bounce</p>
                      <p className={`font-semibold text-sm ${profile.bounce_rate <= 2 ? 'text-green-600' : profile.bounce_rate <= 5 ? 'text-yellow-600' : 'text-red-600'}`}>
                ...
                <CardFooter className="border-t bg-muted/50 px-6 py-3">
                  <div className="flex items-center justify-between w-full text-xs text-muted-foreground">
                    <span>{profile.total_emails_sent.toLocaleString()} enviados</span>
                    <Badge variant="outline" className="text-xs">
                      {profile.current_strategy === 'ramp_up' && 'Ramp-Up'}
                      {profile.current_strategy === 'batch' && 'Batch'}
                      {profile.current_strategy === 'conservative' && 'Conservadora'}
                    </Badge>
                  </div>
                </CardFooter>
              </Card>
