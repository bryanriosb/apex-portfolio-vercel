...
switch (strategy.strategy_type) {
  case 'ramp_up':
  case 'conservative':
    batches = this.calculateRampUpBatches(clients, executionId, strategy, reputation)
    break
  
  case 'batch':
  case 'aggressive':
    batches = this.calculateBatchBatches(clients, executionId, strategy, options)
    break
}
```
...
function calculateBatchBatches(clients, executionId, strategy, options) {
  ...
  while (clientIndex < clients.length) {
    ...
    const batchClients = clients.slice(clientIndex, clientIndex + currentBatchSize)
    
    // Calcular intervalo (primer batch inmediato, demás con delay)
    const interval = options?.customIntervals?.[batchNumber - 1] ??
