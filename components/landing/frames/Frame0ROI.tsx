...
export const Frame0ROI: React.FC<Frame0ROIProps> = () => {
  return (
    <div className="w-full flex items-center justify-center relative animate-in fade-in duration-500">
      ...
      <div className="absolute -top-20 -left-20 w-40 h-40 bg-primary/10 rounded-full blur-3xl pointer-events-none animate-pulse" />
      <div className="absolute -bottom-20 -right-20 w-60 h-60 bg-primary/5 rounded-full blur-3xl pointer-events-none animate-pulse [animation-duration:6s]" />

      <div className="max-w-5xl bg-white border-4 border-gray-900 p-6 sm:p-10 md:p-16 relative overflow-hidden text-center mx-4 sm:mx-0 shadow-[20px_20px_0px_#000] hover:shadow-[25px_25px_0px_#1dcd9f] transition-all duration-300">
        {/* Corner accents */}
        <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-primary/10 to-transparent opacity-0 animate-in fade-in duration-500 delay-1000 fill-mode-forwards" />
        ...
        <h1 className="grid gap-2 sm:gap-3 text-3xl sm:text-5xl md:text-8xl font-black text-gray-900 mb-6 sm:mb-8 leading-[0.85] tracking-tighter uppercase text-left">
          <span className="animate-bounce slide-in-from-bottom-4 duration-700 fill-mode-forwards">
            <CyclingTechnicalTypewriter
              texts={['AUTOMATIZACIÓN', 'TRAZABILIDAD', 'CONTROL', 'PROCESOS']}
              delay={500}
              interval={5000}
            />
          </span>
          <span className="text-primary animate-in slide-in-from-bottom-4 duration-700 delay-300 fill-mode-forwards">
            <CyclingTechnicalTypewriter
              texts={['AGENTES IA', 'OPERACIONES', 'EMPRESARIAL', 'MEDIBLES']}
              delay={1500}
              interval={5000}
            />
          </span>
