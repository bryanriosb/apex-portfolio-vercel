...
interface Frame5GobernanceProps {
  frameVariants?: never
  childVariants?: never
  cardEntranceVariants?: never
}

export const Frame5Gobernance: React.FC<Frame5GobernanceProps> = () => {
  const cards = [
    {
      icon: <Scale />,
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute -top-32 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-gradient-to-b from-primary/5 to-transparent rounded-full blur-3xl pointer-events-none animate-pulse [animation-duration:10s]"
      />

      <div
        className="max-w-3xl mb-8 sm:mb-12 bg-white border-4 border-gray-900 p-6 sm:p-10 shadow-[15px_15px_0px_#000] relative overflow-hidden group mx-4 sm:mx-0 hover:shadow-[20px_20px_0px_#1dcd9f] transition-all duration-300"
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-0">
        {cards.map((card, i) => (
          ...
          >
            <div
              className="h-full hover:shadow-[15px_15px_0px_#1dcd9f] transition-all duration-300"
            >
              <TrustCard {...card} />
            </div>
          </div>
      ...
      >
        {[
          ...
          <div
            key={item}
            className="flex items-center gap-2 px-4 py-2 bg-gray-100 border-2 border-gray-900 text-xs font-black uppercase text-gray-900 hover:bg-primary hover:text-white hover:scale-105 transition-all duration-300 cursor-pointer"
          >
