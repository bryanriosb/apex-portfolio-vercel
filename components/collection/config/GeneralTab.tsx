...
import { getCollectionConfigAction, updateCollectionConfigAction } from '@/lib/actions/collection/config'
...
export function GeneralTab() {
        ...
        const loadConfig = async () => {
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    const isKnownInput = INPUT_DATE_FORMATS.some(f => f.value === loadedInput)
                    ...
                    const isKnownOutput = OUTPUT_DATE_FORMATS.some(f => f.value === loadedOutput)
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true)
        try {
            const finalInputFormat = inputFormat === 'custom' ? customInputFormat : inputFormat
            const finalOutputFormat = outputFormat === 'custom' ? customOutputFormat : outputFormat

            if (inputFormat === 'custom' && !customInputFormat.trim()) {
                toast.error('Debe especificar un formato de entrada personalizado or volver a una opcion predeterminada')
                setIsSaving(false)
                return
            }

            if (outputFormat === 'custom' && !customOutputFormat.trim()) {
                toast.error('Debe especificar un formato de salida personalizado or volver a una opcion predeterminada')
                setIsSaving(false)
                return
            }
            ...
            const result = await updateCollectionConfigAction(activeBusiness.id, updateData)
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Configura cómo el sistema debe interpretar las fechas en tus archivos CSV/Excel de facturas y cómo quieres que se muestren.
                </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-4 max-w-lg">
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                            Selecciona el formato que usualmente tienen las fechas (`invoice_date`, `due_date`) en tus archivos.
                            Si seleccionas "Detectar automáticamente", intentaremos inferir el formato.
                        </p>
                        ...
                        {inputFormat === 'custom' && (
                            <div className="mt-2 text-sm">
                                ...
                                <p className="text-xs text-muted-foreground mt-1 text-wrap sm:w-[350px]">
                                    Use símbolos de <a href="https://date-fns.org/v3.6.0/docs/format" target="_blank" className="underline text-blue-500 hover:text-blue-700">date-fns format</a>.
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-2 pt-4">
                        ...
                        <p className="text-sm text-muted-foreground">
                            Así se mostrarán las fechas en el sistema y correos (ej. templates y previsualización).
                        </p>
