  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        position: 'relative',
        overflow: 'hidden',
        boxShadow: '20px 20px 0px rgba(29, 205, 159, 0.1)',
        borderRadius: 0,
      }}>
        {/* Wizard Header */}
        <div style={{
          height: isMobile ? 50 : 80,
          borderBottom: '2px solid #1dcd9f10',
          display: 'flex',
          alignItems: 'center',
            ...
            {steps.map((step, i) => (
              <div key={step.id} style={{
                ...
                <div style={{
                  ...
                  border: `2px solid ${section >= i ? '#1dcd9f' : '#ccc'}`,
                  backgroundColor: section === i ? '#1dcd9f' : 'transparent',
                  color: section === i ? 'white' : '#1dcd9f',
                ...
                {!isMobile && (
                  <div style={{display: 'flex', flexDirection: 'column'}}>
                    <span style={{fontSize: 10, fontWeight: 900, color: '#1dcd9f', textTransform: 'uppercase', letterSpacing: 1}}>{step.label}</span>
                  </div>
                )}
              </div>
            ))}
          ...
          {!isMobile && (
            <div style={{fontSize: 10, fontWeight: 900, color: '#1dcd9f', opacity: 0.5, letterSpacing: 2}}>APEX_OPERATIONS_CORE</div>
          )}
        </div>
          ...
          {section === 0 && (
            <div style={{display: 'flex', flexDirection: 'column', gap: isMobile ? 15 : 30, height: '100%'}}>
               <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                 ...
                 {!isMobile && (
                   <div style={{fontSize: 10, color: '#1dcd9f', fontWeight: 900}}>[ STATUS: ANALIZANDO_PATRONES_ML ]</div>
                 )}
               </div>
                  ...
                  {gridItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} style={{
                        backgroundColor: 'white', 
                        border: `1px solid ${isVisible ? (item.isValid ? '#1dcd9f20' : '#FF3B3040') : '#1dcd9f05'}`,
                        ...
                        <div style={{width: '60%', height: isMobile ? 2 : 4, backgroundColor: isVisible ? (item.isValid ? '#1dcd9f' : '#FF3B30') : '#1dcd9f10', opacity: 0.8}} />
                        {!isMobile && <div style={{width: '40%', height: 2, backgroundColor: '#1dcd9f10'}} />}
                      </div>
                    );
                  })}
               </div>

               <div style={{display: 'flex', gap: isMobile ? 20 : 40, borderTop: '2px solid #1dcd9f10', paddingTop: isMobile ? 15 : 30}}>
                 <div style={{flex: 1}}>
                 ...
                 {!isMobile && (
                   <>
                     <div style={{flex: 1}}>
                       ...
                       <div style={{fontSize: 24, fontWeight: 900, color: '#1dcd9f'}}>99.8%</div>
                     </div>
                     <div style={{flex: 1}}>
          ...
          {section === 1 && (
            <div style={{height: '100%', display: 'flex', flexDirection: 'column', gap: isMobile ? 20 : 40}}>
               <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                 ...
                 {!isMobile && (
                   <div style={{fontSize: 10, color: '#1dcd9f', fontWeight: 900}}>[ BATCH_ENGINE: ACTIVE ]</div>
                 )}
               </div>
               
               <div style={{flex: 1, display: 'flex', alignItems: 'flex-end', gap: isMobile ? 4 : 8, border: '1px solid #1dcd9f10', padding: isMobile ? 15 : 30, backgroundColor: 'white'}}>
                  {barHeights.map((h, i) => (
                    <div key={i} style={{
                      flex: 1,
                      height: `${h}%`,
               ...
               {!isMobile && (
                 <div style={{display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#1dcd9f', fontWeight: 900, borderTop: '2px solid #1dcd9f10', paddingTop: 20}}>
          ...
          {section >= 2 && (
            <div style={{height: '100%', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.4fr 1fr', gap: isMobile ? 20 : 40}}>
               <div style={{display: 'flex', flexDirection: 'column', gap: isMobile ? 15 : 30}}>
                 <div style={{fontSize: isMobile ? 12 : 14, fontWeight: 900, color: '#1dcd9f', letterSpacing: -0.5}}>TELEMETRÍA_REALTIME</div>
                 <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: isMobile ? 10 : 20, flex: 1}}>
                    {[
                      ...
                      <div key={s.l} style={{padding: isMobile ? 15 : 25, border: '2px solid #1dcd9f', backgroundColor: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
               ...
               {!isMobile && (
                 <div style={{border: '2px solid #1dcd9f10', backgroundColor: 'white', padding: 30, display: 'flex', flexDirection: 'column'}}>
                   <div style={{fontSize: 10, fontWeight: 900, color: '#1dcd9f', marginBottom: 20, letterSpacing: 1}}>SISTEMA_TRACE_LOG</div>
                   <div style={{flex: 1, fontSize: 9, color: '#666', lineHeight: 2.2, overflow: 'hidden', fontWeight: 600}}>
                      ...
                      <div style={{color: '#1dcd9f'}}>{`> [${new Date().toLocaleTimeString()}] AGENT: OPTIMIZING_NEXT_BATCH...`}</div>
                   </div>
                 </div>
               )}
            </div>
